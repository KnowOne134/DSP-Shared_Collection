...
function onTrigger(player, npc)
    ...
    if not player:hasKeyItem(dsp.ki.RUNIC_DISC) then
        player:messageSpecial(ID.text.NEW_USER, dsp.ki.RUNIC_DISC)
        npcUtil.giveKeyItem(player, dsp.ki.RUNIC_DISC)
    elseif instance:getLocalVar("runeHandler") ~= 0 then
        ...
        player:startEvent(94, dsp.ki.RUNIC_DISC, tokens, 1, prefered, 100, 200, 300, floorProgress)
	end
end
