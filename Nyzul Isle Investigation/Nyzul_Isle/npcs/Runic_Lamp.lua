...
function onEventFinish(player, csid, option, npc)
    ...
    if csid == 3 and option == 1 then
        if OBJECTIVE == nyzul.lampsObjective.ACTIVATE_ALL then
            ...
            if instance:getEntity(bit.band(ID.npc.RUNIC_LAMP_1, 0xFFF), dsp.objType.NPC):AnimationSub() == 1 and
            instance:getEntity(bit.band(ID.npc.RUNIC_LAMP_2, 0xFFF), dsp.objType.NPC):AnimationSub() == 1 and
            instance:getEntity(bit.band(ID.npc.RUNIC_LAMP_3, 0xFFF), dsp.objType.NPC):AnimationSub() == 1 then
                if lampCount == 3 then
                    instance:setProgress(15)
                elseif instance:getEntity(bit.band(ID.npc.RUNIC_LAMP_4, 0xFFF), dsp.objType.NPC):AnimationSub() == 1 then
                    if lampCount == 4 then
                        instance:setProgress(15)
                    elseif instance:getEntity(bit.band(ID.npc.RUNIC_LAMP_5, 0xFFF), dsp.objType.NPC):AnimationSub() == 1 then
                        instance:setProgress(15)
                    end
                end
            end
...
--            print("lamp count: "..instance:getLocalVar("[Lamp]count").." press count: "..instance:getLocalVar("[Lamp]pressCount"))
            ...
            if lampCount == 3 and lampRegister > 13 then
                for i = ID.npc.RUNIC_LAMP_1, ID.npc.RUNIC_LAMP_3 do
                    local lamp = instance:getEntity(bit.band(i, 0xFFF), dsp.objType.NPC)
