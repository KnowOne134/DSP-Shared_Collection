...
quest.sections = {
...

    {
        ...
        [dsp.zone.AHT_URHGAN_WHITEGATE] = {
            ...
            ['Ulamaal'] = {
                onTrigger = function(player, npc)
                    if player:hasKeyItem(dsp.ki.RAINBOW_BERRY) then
        ...
        [dsp.zone.WAJAOM_WOODLANDS] = {
            ['Harvesting_Point'] = {
                onTrade = function(player, npc, trade)
                    if npcUtil.tradeHasExactly(trade, dsp.items.SICKLE) and quest:getVar(player, 'Prog') == 2 and not player:hasKeyItem(dsp.ki.RAINBOW_BERRY) then
