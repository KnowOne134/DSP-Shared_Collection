...

    {
        check = function(player, status, vars)
            return status == QUEST_AVAILABLE and
                player:canEquipItem(dsp.items.AXE_OF_TRIALS, true) and
                player:getCharSkillLevel(dsp.skill.GREAT_AXE) / 10 >= 240 and
        ...
        [dsp.zone.METALWORKS] = {
            ...
            onEventFinish = {
                [790] = function(player, csid, option, npc)
                    if npcUtil.giveItem(player, dsp.items.AXE_OF_TRIALS) then
        ...
        [dsp.zone.METALWORKS] = {
            ['Iron_Eater'] = {
                ...
                onTrade = function(player, npc, trade)
                    ...
                    if npcUtil.tradeHasExactly(trade, dsp.items.AXE_OF_TRIALS) then
            ...
            onEventFinish = {
                [791] = function(player, csid, option, npc)
                    if option == 1 then
                        npcUtil.giveItem(player, dsp.items.AXE_OF_TRIALS)
                ...
                [794] = function(player, csid, option, npc)
                    ...
                    player:delKeyItem(dsp.ki.WEAPON_TRAINING_GUIDE)
                    player:addLearnedWeaponskill(dsp.ws_unlock.STEEL_CYCLONE)
        ...
        [dsp.zone.THE_SANCTUARY_OF_ZITAH] = {
            ['qm1'] = {
                onTrigger = function(player, npc)
                    ...
                    elseif player:hasKeyItem(dsp.ki.MAP_TO_THE_ANNALS_OF_TRUTH) and not player:hasKeyItem(dsp.keyItem.ANNALS_OF_TRUTH) and npcUtil.popFromQM(player, npc, zones[player:getZoneID()].mob.GREENMAN, {hide = 0}) then
