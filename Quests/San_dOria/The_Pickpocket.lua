...
local quest = Quest:new(SANDORIA, THE_PICKPOCKET)

quest.reward = {
    fame = 30,
    item = dsp.items.LIGHT_AXE,
    title = dsp.title.PICKPOCKET_PINCHER,
}
...
quest.sections = {
        ...
        [dsp.zone.PORT_SAN_DORIA] = {
            ['Altiret'] = {
                ...
                onTrade = function(player, npc, trade)
                    if npcUtil.tradeHasExactly(trade, dsp.items.GILT_GLASSES) then
            ...
            ['Miene'] = {
                onTrigger = function(player, npc)
                    ...
                    if stage == 0 then
                        ...
                        if player:getFreeSlotsCount() > 0 and not player:hasItem(dsp.items.EAGLE_BUTTON) and not player:hasItem(dsp.items.GILT_GLASSES) then
                            -- Reaquire the button
            ...
            onEventFinish = {
                [549] = function(player, csid, option, npc)
                    if npcUtil.giveItem(player, dsp.items.EAGLE_BUTTON) then
                        quest:setVar(player, 'Stage', 1)
                    end
                end,

                [611] = function(player, csid, option, npc)
                    npcUtil.giveItem(player, dsp.items.EAGLE_BUTTON)
                end,
        ...
        [dsp.zone.WEST_RONFAURE] = {
            ['Esca'] = {
                onTrigger = function(player, npc, trade)
                    if player:hasItem(dsp.items.GILT_GLASSES) then
                ...
                onTrade = function(player, npc, trade)
                    if npcUtil.tradeHasExactly(trade, dsp.items.EAGLE_BUTTON) then
            ...
            onEventFinish = {
                [121] = function(player, csid, option, npc)
                    if npcUtil.giveItem(player, dsp.items.GILT_GLASSES) then
                        player:confirmTrade()
                    end
                end,
            },
    ...
    -- Section: Hints during questing
    {
        check = function(player, status, vars)
            return (status == QUEST_AVAILABLE and vars.Prog == 1)
                or (status == QUEST_ACCEPTED and not player:hasItem(dsp.items.GILT_GLASSES))
        end,
