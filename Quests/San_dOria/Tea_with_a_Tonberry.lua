        ...
        [dsp.zone.CARPENTERS_LANDING] = {
            ['Anguenet'] = {
                onTrigger = function(player, npc)
                    ...
                    if progress == 0 then
                        return quest:progressEvent(27, { [1] = dsp.items.PIECE_OF_ATTOHWA_GINSENG })
                    elseif progress == 1 then
                        return quest:replaceEvent(28, { [1] = dsp.items.PIECE_OF_ATTOHWA_GINSENG })
                    end
                end,

                onTrade = function(player, npc, trade)
                    if quest:getVar(player, 'Prog') == 1
                        and npcUtil.tradeHas(trade, dsp.items.PIECE_OF_ATTOHWA_GINSENG) then
            ...
            onEventFinish = {
                ...
                [29] = function(player, csid, option, npc)
                    ...
                    player:addKeyItem(dsp.keyItem.TONBERRY_BLACKBOARD)
                    player:messageSpecial(CarpentersLandingID.text.KEYITEM_OBTAINED, dsp.keyItem.TONBERRY_BLACKBOARD)
                    quest:setVar(player, 'Prog', 2)
                end,
            },
        },

        [dsp.zone.PHANAUET_CHANNEL] = {
            ['Riche'] = {
                onTrigger = function(player, npc)
                    if player:hasKeyItem(dsp.keyItem.TONBERRY_BLACKBOARD) then
                        return quest:progressEvent(5, 1, 627, dsp.items.INGOT_OF_ROYAL_TREASURY_GOLD, 63, 3, 30, 30, 0)
            ...
            onEventFinish = {
                [5] = function(player, csid, option, npc)
                    ...
                    player:delKeyItem(dsp.keyItem.TONBERRY_BLACKBOARD) -- No message as the cutscene ends with the NPC taking it
                end,
            },
        },
            ...
            ['qm2'] = {
                onTrigger = function(player, npc)
                    ...
                    elseif progress == 4 then
                        return quest:event(126, 149, dsp.items.INGOT_OF_ROYAL_TREASURY_GOLD)
                ...
                onTrade = function(player, npc, trade)
                    if npcUtil.tradeHas(trade, dsp.items.INGOT_OF_ROYAL_TREASURY_GOLD)
