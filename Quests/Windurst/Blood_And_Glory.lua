...

    {
        check = function(player, status, vars)
            return status == QUEST_AVAILABLE and
                player:canEquipItem(dsp.items.POLE_OF_TRIALS, true) and
                player:getCharSkillLevel(dsp.skill.STAFF) / 10 >= 230 and
        ...
        [dsp.zone.WINDURST_WALLS] = {
            ...
            onEventFinish = {
                [445] = function(player, csid, option, npc)
                    if npcUtil.giveItem(player, dsp.items.POLE_OF_TRIALS) then
        ...
        [dsp.zone.WINDURST_WALLS] = {
            ['Shantotto'] = {
                ...
                onTrade = function(player, npc, trade)
                    ...
                    if npcUtil.tradeHasExactly(trade, dsp.items.POLE_OF_TRIALS) then
            ...
            onEventFinish = {
                [446] = function(player, csid, option, npc)
                    if option == 1 then
                        npcUtil.giveItem(player, dsp.items.POLE_OF_TRIALS)
                ...
                [450] = function(player, csid, option, npc)
                    ...
                    player:delKeyItem(dsp.ki.WEAPON_TRAINING_GUIDE)
                    player:addLearnedWeaponskill(dsp.ws_unlock.RETRIBUTION)
        ...
        [dsp.zone.IFRITS_CAULDRON] = {
            ['qm3'] = {
                onTrigger = function(player, npc)
                    ...
                    elseif player:hasKeyItem(dsp.ki.MAP_TO_THE_ANNALS_OF_TRUTH) and not player:hasKeyItem(dsp.keyItem.ANNALS_OF_TRUTH) and npcUtil.popFromQM(player, npc, zones[player:getZoneID()].mob.CAILLEACH_BHEUR, {hide = 0}) then
