...
quest.sections = {
    ...
    -- Section: Questing
    {
        ...
        [dsp.zone.UPPER_JEUNO] = {
            ['Luto_Mewrilah'] = {
                onTrigger = function(player, npc)
                    if quest:getVar(player, "Prog") == 1 and player:hasKeyItem(dsp.keyItem.VITRALLUM) then
                        local fellowParam = getFellowParam(player)
        ...
        [dsp.zone.KORROLOKA_TUNNEL] = {
            ['qm3'] = {
                onTrigger = function(player, npc)
                    if not player:hasKeyItem(dsp.keyItem.VITRALLUM) then
                        ...
                        elseif quest:getVar(player, "Prog") == 1 then
                            npcUtil.giveKeyItem(player, dsp.keyItem.VITRALLUM)
                        else
                            return quest:messageSpecial(zones[player:getZoneID()].text.NOTHING_OUT_OF_ORDINARY)
                        end
                    else
        ...
        [dsp.zone.JUGNER_FOREST] = {
            ...
            onEventFinish = {
                [31] = function(player, csid, option, npc)
                    ...
                    player:delKeyItem(dsp.keyItem.VITRALLUM)
