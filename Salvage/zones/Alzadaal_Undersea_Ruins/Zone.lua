...
function onInitialize(zone)
    ...
    zone:registerRegion(24, 54, -1, 749, 65, 1, 758)    -- Transformations quest part 1
    zone:registerRegion(25, 135,-1, -557,143,1, -548)  -- Transformations quest part 2
end
...
function onZoneIn(player,prevZone)
    ...
    if player:getXPos() == 0 and player:getYPos() == 0 and player:getZPos() == 0 then
        ...
        elseif prevZone == dsp.zone.ZHAYOLM_REMNANTS then
            player:setPos(-580, -0.5, -433, 64)
        elseif prevZone == dsp.zone.BHAFLAU_REMNANTS then
...
function onEventFinish(player,csid,option)
    if csid == 1 then
        npcUtil.giveKeyItem(player, dsp.ki.ASTRAL_COMPASS)
    ...
    elseif csid == 411 then
        ...
        if area == 5 then
            player:setPos(0, 0, 0, 0, dsp.zone.NYZUL_ISLE)
        elseif area == 7 then
